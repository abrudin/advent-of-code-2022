fun day12(input: String): Pair<Int, Int> {
    ...
    data class Hill(val current: Set<Int>, val previous: Set<Int> = setOf(), val steps: Int = 0)

    fun stepsUntilEnd(init: Set<Int>): Int = generateSequence(Hill(init)) { hill ->
        Hill(hill.current.flatMap { i ->
            listOfNotNull(
        ...
        }.toSet(), hill.previous + hill.current, hill.steps.inc())
